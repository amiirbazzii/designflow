...
export function CompactView({
  ...
  return <Box flexGrow={1} flexDirection="column" paddingX={1}>
    ...
    {navigation.view === "output-viewer" ? <OutputViewer output={selectedOutputView} document={viewerDocument} scrollOffset={navigation.outputScrollOffset} visibleLines={viewerVisibleLines} details={navigation.outputDetails} />
      ...
      : navigation.view === "ready-to-generate" && navigation.freshState !== undefined ? navigation.freshEvidence === undefined ? <Box flexDirection="column"><Text bold>Ready to generate</Text><Text color={designFlowTheme.success}>✓ {navigation.freshState.design.label}</Text><Text color={designFlowTheme.textSecondary}>Frame node: {navigation.freshState.nodeId}</Text>{navigation.loading === "fresh-evidence" && <Text color={designFlowTheme.accent}>Retrieving Figma evidence…</Text>}<Text color={designFlowTheme.accentStrong}>No generation started.</Text></Box> : <Box flexDirection="column"><Text bold>Figma evidence ready</Text><Text color={designFlowTheme.success}>✓ {navigation.freshEvidence.frame.name}</Text><Text color={designFlowTheme.textSecondary}>Frame node: {navigation.freshEvidence.frame.id}</Text><Text color={designFlowTheme.textSecondary}>Size: {navigation.freshEvidence.frame.width} × {navigation.freshEvidence.frame.height}</Text>{navigation.loading === "fresh-scaffold" && <Text color={designFlowTheme.accent}>Creating project scaffold…</Text>}{navigation.freshProject !== undefined && <Text color={designFlowTheme.success}>Project: {navigation.freshProject.targetPath}</Text>}<Text color={designFlowTheme.accentStrong}>No generation started.</Text></Box>
