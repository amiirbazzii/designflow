...
export const implementationAgentManifest: AgentManifest = agentManifestSchema.parse({
  ...
  instructions:
    ...
    "File scope is exactly four categories: EXISTING AND REUSABLE (paths in trustedReusePaths — reference them via imports and existing_reuse claims), EXISTING AND MODIFIABLE (paths listed in the project context — action 'modify' only), CREATE-ALLOWED (new project-relative paths — action 'create'), and READ-ONLY (every other path — never propose or claim it; the host will deterministically reject it).",
