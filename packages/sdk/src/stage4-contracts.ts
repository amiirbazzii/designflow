...
export const implementationValidationReportSchema = z.object({
  schemaVersion: z.literal("1"), projectId: z.string().min(1), proposalArtifactId: z.string().min(1), applicationArtifactId: z.string().min(1), checks: z.array(z.object({ name: z.enum(["format", "typecheck", "lint", "build", "test"]), status: z.enum(["passed", "failed", "skipped", "unavailable"]), required: z.boolean(), commandReference: z.string().optional(), exitCode: z.number().int().optional(), durationMs: z.number().nonnegative().optional(), logArtifactId: z.string().optional(), summary: z.string().min(1) }).strict()), passed: z.boolean(), rollbackTriggered: z.boolean(), rollbackArtifactId: z.string().optional(), warnings: z.array(z.string()),
}).strict();
