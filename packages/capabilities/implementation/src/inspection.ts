...
function command(name: "format"|"typecheck"|"lint"|"build"|"test", script: unknown, required: boolean) {
  if (typeof script !== "string" || !/^[a-zA-Z0-9_.:@/-]+(?:\s+[a-zA-Z0-9_.:@/-]+)*$/.test(script)) return undefined;
  const [executable, ...args] = script.split(/\s+/); if (!executable || !["bun", "npm", "pnpm", "yarn", "node", "prettier", "eslint", "tsc", "vitest", "jest"].includes(executable)) return undefined;
  return { name, executable, args, source: "package-script" as const, required };
}
...
export function inspectRegisteredProject(project: { id: string; name: string; rootPath?: string }, limits: InspectionLimits = {}, signal?: AbortSignal): Stage4ProjectImplementationContext {
  ...
  const commands = { format: command("format", scripts.format, false), typecheck: command("typecheck", scripts.typecheck, true), lint: command("lint", scripts.lint, true), build: command("build", scripts.build, true), test: command("test", scripts.test, false) };
  const contextBase = { schemaVersion: "1" as const, project: { id: project.id, rootIdentity: hash(root), contextFingerprint: "pending" }, runtime: { framework, ...(dependencies[framework] ? { frameworkVersion: dependencies[framework] } : {}), language, packageManager: lstatSync(join(root, "bun.lock"), { throwIfNoEntry: false }) ? "bun" : lstatSync(join(root, "pnpm-lock.yaml"), { throwIfNoEntry: false }) ? "pnpm" : lstatSync(join(root, "yarn.lock"), { throwIfNoEntry: false }) ? "yarn" : "npm", monorepo: Boolean(pkg.workspaces) }, structure: { sourceRoots: [...new Set(sourceRoots)].sort(), routeRoots: files.filter((file) => /routes?|pages?|app\//.test(file)).map((file) => file.split("/").slice(0, -1).join("/")).filter((v,i,a)=>v&&a.indexOf(v)===i).sort(), publicAssetRoots: files.filter((file) => /^(public|static|assets)\//.test(file)).map((file)=>file.split("/")[0]).filter((v,i,a)=>a.indexOf(v)===i).sort(), aliases: {} }, styling: { strategies: strategies.length ? strategies : ["unknown"], ...(strategies[0] ? { primaryStrategy: strategies[0] } : {}), evidence: strategies }, designSystem: { tokenSources: tokens.length ? [{ path: tokens[0]!.sourcePath, kind: "css-variables", evidence: ["CSS custom properties detected"] }] : [], tokens, componentSources: components.map((component) => ({ path: component.sourcePath, exportedNames: [component.name] })), components }, conventions: { naming: ["component names are PascalCase when exported"], fileLayout: sourceRoots.length ? [`source files live under ${sourceRoots[0]}`] : [], exports: ["preserve existing export style"], props: ["reuse existing prop types when present"], testing: files.some((file)=>/\.test\.|\.spec\./.test(file)) ? ["tests are colocated"] : [], accessibility: ["preserve semantic roles and accessible names"] }, commands, warnings };
