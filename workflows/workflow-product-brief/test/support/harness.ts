// workflows/workflow-product-brief/test/support/harness.ts
import {
...
import { productBriefWorkflowPackage } from "../../src/manifest";
...
export interface ProductBriefHost {
...
export function createHost(options?: {
  ...
  productBriefWorkflowPackage.load(capabilityRegistry);

  const workflows = new Map<string, WorkflowPackage>([
    [productBriefWorkflowPackage.id, productBriefWorkflowPackage],
  ]);
...
/**
 * A request with four explicit asks — one of each priority signal (must,
 * should, nice-to-have, unmarked) — plus a constraint and one hedge word in
 * each of the requirement text and the constraints, so every deterministic
 * rule in the workflow has something to fire on.
 */
export const SAMPLE_REQUEST = {
  productRequest: [
    "Users must be able to reset their password from the login screen",
    "The team should add audit logging for admin actions",
    "It would be nice to have a dark mode toggle",
    "It is unclear whether guest users need self-service reset too",
  ].join("\n"),
  targetUser: "self-service account holders",
  whyItMatters: "Support tickets for locked-out accounts are the top complaint",
  constraints: ["Ship within the current quarter", "Scope may expand after v1"],
  desiredOutputScope: [],
  excludedScope: ["Social login providers"],
};
